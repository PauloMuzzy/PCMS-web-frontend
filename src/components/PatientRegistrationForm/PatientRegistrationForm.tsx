...
export function PatientRegistrationForm() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors }
  } = useForm<CreatePatientFormData>({resolver: zodResolver(createPatientFormSchema)})

  const registerWithMask = useHookFormMask(register);

  const onSubmit: SubmitHandler<CreatePatientFormData> = async (data: CreatePatientFormData): Promise<void> => {
    setIsLoading(true)
    try {
